...
对于一个带符号的数来说，正数三码合一(原码、反码、补码都一样)；

负数的反码为其原码除符号位以外的各位按位取反

负数的补码是先取反然后加一，不要忘了负数的符号位为1

|十进制数|原码|反码|补码|说明|
|----|----|----|----|----|
|1|0000 0001|0000 0001|0000 0001|正数三码合一|
|-1|1000 0001|1111 1110|1111 1111|负数补码=反码+1|
...
X的求补(运算)为：neg(X) = 反数+1 = (-X)的补码  【neg是求补运算】

反数和反码的区别：我们以二进制来讲：反数实际就是对原码的每一位进行包括符号位在内进行取反，即0变1，1变0

求补运算与补码的区别在于，求补运算时不考虑是否有符号位，所有的位都要取反，最后加一，它求得的结果
不是求这个数的补码，而是这个数相反数的补码。
