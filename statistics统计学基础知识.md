...
* [1. 常见概念](#1-)
   * [1. 置信区间](#1-)
   * [2. 标准差(SD)和标准误(SE)](#2-sdse)
   * [3. 误差(Error)，偏差(Bias)，方差(Variance)](#3-errorbiasvariance)
   * [4. 医学统计学中的OR，HR，RR](#4-orhrrr)
   * [5. 医学统计学中的三大多变量回归分析：Cox 回归、logistic 回归和多元线性回归](#5-cox-logistic-)
   * [6. pearson、spearman和kendall相关性分析](#6-pearsonspearmankendall)
...
* [3. 各种分布](#3-)
   * [1. 泊松分布](#1-)
   * [2. 二项分布(Binomial distribution, 贝努里分布)](#2-binomial-distribution-)
   * [3. 负二项分布(negative binomial distribution)](#3-negative-binomial-distribution)
   * [4. beta分布](#4-beta)
   * [一句话说分布](#)
* [4. 统计检验](#4-)
   * [1. 卡方检验](#1-)
...
而“负二项分布”是所有到成功r次时即终止的独立试验中，失败次数k的分布。

![](http://latex.codecogs.com/gif.latex?f(k|r,p)=\\binom{k+r-1}{r-1}\\cdot&space;p^{r}\\cdot&space;(1-p)^k)

k:失败的次数，r:成功的次数，p:成功的概率

它表示，已知一个事件在伯努利试验中每次的出现概率是p，在一连串伯努利试验中，一件事件刚好在第r+k次
试验出现第r次时出现k次的概率。
